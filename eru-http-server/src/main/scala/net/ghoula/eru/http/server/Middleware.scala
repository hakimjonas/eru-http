...
object Middleware {
  ...
  /** Request body size limits: answers 413 before the handler runs when the request declares a
    * Content-Length over the matching limit. `perContentTypePrefix` keys are lowercase Content-Type
    * prefixes (for example "image/" or "application/json"), matched before the default. Requests
    * without a Content-Length (chunked framing) pass through — their enforcement is incremental:
    * `maxRequestSize` bounds the stream and framing failures surface as 400.
    *
    * Enforcement point note: the parser buffers Content-Length bodies up to `maxRequestSize` before
    * the middleware runs, so this middleware decides the status code, it does not reduce the peak
    * memory of an oversized declared body — `maxRequestSize` remains the hard memory bound.
    *
    * @param defaultMaxBytes
    *   the limit applied when no per-type prefix matches
    * @param perContentTypePrefix
    *   per-content-type limits, keyed by lowercase type prefix
    */
  def bodyLimit(
    defaultMaxBytes: Long,
    perContentTypePrefix: Map[String, Long] = Map.empty
  ): Middleware = handler =>
    req => {
      val declared =
        req.headers.getFirst(HeaderNames.ContentLength).flatMap(h => h.value.trim.toLongOption)
      val contentType =
        req.headers.getFirst(HeaderNames.ContentType).map(_.value.trim.toLowerCase).getOrElse("")
      val limit = perContentTypePrefix.collectFirst {
        case (prefix, max) if contentType.startsWith(prefix) => max
      }.getOrElse(defaultMaxBytes)
      declared match {
        case Some(len) if len > limit =>
          Eru.succeed(
            Response(
              status = StatusCode.ContentTooLarge,
              headers = Headers.empty,
              body = Body.text(s"Request body of $len bytes exceeds the limit of $limit bytes")
            )
          )
        case _ => handler(req)
      }
  ...
  /** Response compression middleware.
    ...
    * and brotli compression. Streaming responses without a Content-Length skip the `minSize` check
    * but are still subject to Content-Encoding presence and Accept-Encoding negotiation.
    *
    * @param config
  ...
  private def chooseEncoding(
    ...
    preferred.find { enc =>
      val clientAccepts = accepted.contains(enc.value.toLowerCase) || accepted.contains("*")
      clientAccepts && Compression.isSupported(enc)
    }
  }
...
/** CORS configuration.
  ...
  *   origins permitted by the CORS check; `*` allows any origin
  * @param allowedMethods
  *   methods permitted in preflight responses
  * @param allowedHeaders
  *   request headers permitted in preflight responses
  * @param allowCredentials
  *   whether to emit `Access-Control-Allow-Credentials: true`
  * @param maxAge
  *   preflight cache lifetime in seconds; the default of 86400 is 24 hours
  */
