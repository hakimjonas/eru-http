...
class HpackHuffmanSpec extends FunSuite {
  ...
  test("RFC 7541 C.4.1: Literal Header Field with Indexing (custom-key)") {
    // "custom-key" Huffman encoded should be: 25a849e95ba97d7f (RFC 7541 Appendix C.4.1)
  ...
  test("Padding uses EOS prefix bits") {
    // When encoding doesn't end on a byte boundary, padding should be EOS MSBs. 'a' is 5 bits
    // (00011) and needs 3 bits of padding: 'a' (00011) + padding (111) = 00011111 = 0x1f.
    val input = "a".getBytes("UTF-8")
    val buffer = ByteBuffer.allocate(10)
