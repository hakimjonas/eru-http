...
class H2ServerConnectionSpec extends FunSuite {
  ...
  // ============================================================================
  // CONTINUATION Flood Protection (CVE-2024-27316)
  // ============================================================================

  test("readContinuationFrames rejects header list exceeding maxHeaderListSize") {
    val channel = new MockChannel()
    val settings = H2Settings.create(maxHeaderListSize = 64).assertSuccess
    val conn = H2ServerConnection(channel, settings).assertSuccess

    // Build a HEADERS frame WITHOUT END_HEADERS (triggers CONTINUATION reading)
    val initialBlock = new Array[Byte](32)
    val headersFrame = H2FrameCodec.headersFrame(1, initialBlock, endStream = true, endHeaders = false)
    val headersEncoded = H2FrameCodec.encode(headersFrame)
    val headersBytes = new Array[Byte](headersEncoded.remaining)
    headersEncoded.get(headersBytes)

    // Build a CONTINUATION frame that pushes total over the limit (32 + 50 = 82 > 64)
    val contBlock = new Array[Byte](50)
    val contFrame = H2FrameCodec.continuationFrame(1, contBlock, endHeaders = false)
    val contEncoded = H2FrameCodec.encode(contFrame)
    val contBytes = new Array[Byte](contEncoded.remaining)
    contEncoded.get(contBytes)

    channel.setReadData(headersBytes ++ contBytes)

    // receiveRequest should fail — either size limit or stream error
    val result = conn.receiveRequest()
    assert(result.isFailure, "Should reject oversized header list")

    // Verify GOAWAY was sent (connection-level error response)
    val written = channel.getWrittenData
    val buffer = java.nio.ByteBuffer.wrap(written)
    var foundGoaway = false
    while buffer.remaining >= H2Frame.HeaderSize && !foundGoaway do {
      val pos = buffer.position
      val length =
        ((buffer.get(pos) & 0xff) << 16) | ((buffer.get(pos + 1) & 0xff) << 8) | (buffer.get(pos + 2) & 0xff)
      val frameType = buffer.get(pos + 3) & 0xff
      if frameType == H2Frame.FrameType.GoAway then foundGoaway = true
      buffer.position(pos + H2Frame.HeaderSize + length): Unit
    }
    assert(foundGoaway, "Server should have sent GOAWAY frame")
  }

  test("default maxHeaderListSize is 64KB") {
    assertEquals(H2Frame.DefaultSettings.MaxHeaderListSize, 65536)
  }
