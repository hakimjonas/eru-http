...
final class H2ClientConnection private (
  ...
  /** Send a request on a new stream.
    ...
    *   host:port or host
    * @param scheme
    *   the :scheme pseudo-header value, "https" by default
    * @param headers
    *   additional headers
    * @param body
  ...
  /** Send DATA frames for request body.
    ...
    * ensure frames are being read concurrently (via receiveResponse) to process WINDOW_UPDATE. When
    * both windows are exhausted, the wait uses the stream window as primary since it is per-stream
    * and more likely to be the bottleneck.
    *
    * @param stream
