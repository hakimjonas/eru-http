...
final class H2Stream private (
  ...
  /** Check if a frame can be sent in the current state.
    *
    * RST_STREAM and PRIORITY can always be sent; WINDOW_UPDATE can be sent on any non-closed
    * stream; CONTINUATION follows the HEADERS rules.
    *
    * @param frameType
  ...
  /** Check if a frame can be received in the current state.
    *
    * RST_STREAM and PRIORITY can always be received; WINDOW_UPDATE can be received on any
    * non-closed stream; CONTINUATION follows the HEADERS rules.
    *
    * @param frameType
...
object H2Stream {
  ...
  /** Create a new stream in idle state.
    *
    * The window semaphore starts at 0, so waiters block until the window becomes available.
    *
    * @param streamId
