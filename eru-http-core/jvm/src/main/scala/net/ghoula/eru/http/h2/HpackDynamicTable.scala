...
final class HpackDynamicTable(initialMaxSize: Int) {
  ...
  def add(name: String, value: String): Unit = {
    ...
    if newEntrySize > maxSize then clear()
    else {
  ...
  def find(name: String, value: String): Option[(Int, Boolean)] = {
    @tailrec
    def loop(i: Int, nameMatchIndex: Option[Int]): Option[(Int, Boolean)] =
      if i >= entries.length then nameMatchIndex.map((_, false))
      else {
        val entry = entries(i)
        if entry.name != name then loop(i + 1, nameMatchIndex)
        else if entry.value == value then Some((HpackStaticTable.size + i + 1, true))
        else loop(i + 1, nameMatchIndex.orElse(Some(HpackStaticTable.size + i + 1)))
      }
  ...
  def findName(name: String): Option[Int] = {
    ...
    def loop(i: Int): Option[Int] =
      if i >= entries.length then None
      else if entries(i).name == name then Some(HpackStaticTable.size + i + 1)
      else loop(i + 1)
