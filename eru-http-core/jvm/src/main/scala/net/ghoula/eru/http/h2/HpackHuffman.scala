...
object HpackHuffman {
  ...
  // Returns are used for early exit from nested loops on error - idiomatic for bit-level encoding
  def encode(input: Array[Byte], buffer: ByteBuffer): Eru[HpackError, Int] = {
      ...
      while bitsRemaining > 0 do {
        ...
        if bitsInCurrentByte == 8 then {
          if buffer.remaining < 1 then {
            return Eru.fail(HpackError.BufferError("Buffer overflow during Huffman encoding"))
          }
    ...
    if bitsInCurrentByte > 0 then {
      ...
      currentByte = (currentByte << paddingBits) | eosMsb

      if buffer.remaining < 1 then {
        return Eru.fail(HpackError.BufferError("Buffer overflow during Huffman padding"))
      }
      buffer.put(currentByte.toByte): Unit
      bytesWritten += 1
    }

    Eru.succeed(bytesWritten)
  }
  ...
  def encodedLength(input: Array[Byte]): Int = {
    ...
    (totalBits + 7) / 8 // Round up to nearest byte
  }
  ...
  def decode(buffer: ByteBuffer, length: Int): Eru[HpackError, Array[Byte]] = {
    if buffer.remaining < length then {
      ...
      var bytesRead = 0
      var eosFound = false // Track EOS error across the entire decode process
      ...
      while bytesRead < length && !eosFound do {
        ...
        var decoded = true
        while decoded && accumulatorBits >= 5 && !eosFound do { // Minimum code length is 5
          decoded = false

          // Search for matching code (brute force - can be optimized with lookup table)
          // Check all 257 symbols including EOS (256)
          var symbol = 0
          while symbol <= 256 && !decoded do {
            ...
            if accumulatorBits >= code.length then {
              ...
              if (candidate & mask) == code.bits then {
                if symbol == EOS then {
                  // EOS symbol in string is a decoding error per RFC 7541 Section 5.2
                  eosFound = true
                  decoded = true
                } else {
      ...
      if eosFound then {
        ...
        // - Padding must consist of the most significant bits of EOS
        // Remaining bits should be the MSB of EOS (all 1s for valid padding)
