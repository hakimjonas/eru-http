...
final class HpackDecoder(maxDynamicTableSize: Int = HpackDynamicTable.DefaultMaxSize) {
  ...
  /** Decode a single header field representation.
    ...
    *
    * Per RFC 7541 Section 4.2, a dynamic table size update must occur at the beginning of the
    * header block, before any header field representation.
    *
    * @return
  ...
  private def decodeRepresentation(
    ...
    if buffer.remaining < 1 then {
      ...
      val firstByte = buffer.get(buffer.position()) & 0xff
