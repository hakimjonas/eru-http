...
object WebSocketFrameWriter {
  ...
  /** Write a WebSocket frame to a channel.
    *
    * The payload is copied byte-by-byte from the frame's underlying array into the send buffer,
    * with XOR masking applied when `mask` is set.
    *
    * @param channel
  ...
  def writeFrame(
    ...
    val payload = frame.payload.unsafeArray
  ...
  /** Write a binary message, fragmenting if necessary.
    *
    * writeData only reads `data`; the frame constructors call Bytes.fromArray, which
    * defensive-copies, so there is no mutation-shared-with-caller hazard.
    *
    * @param channel
    *   the channel to write to
    * @param data
  ...
  def writeBinary(
    ...
    writeData(channel, data.unsafeArray, WebSocketOpcode.Binary, mask, maxFrameSize)
  }
