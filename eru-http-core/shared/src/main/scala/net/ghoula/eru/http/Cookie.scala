...
/** HTTP Cookie as defined in RFC 6265.
  *
  * A cookie with all its attributes as specified in the Set-Cookie header.
  *
  * @param name
...
final case class Cookie(
  ...
  /** Checks if this cookie matches the given path per RFC 6265 Section 5.1.4.
    ...
    *   - Request path must path-match the cookie path (a prefix only matches at a `/` boundary:
    *     either the cookie path ends with `/` or the next request-path character is `/`)
    *
    * @param requestPath
...
object Cookie {
  ...
  /** Parses a Set-Cookie header value per RFC 6265 Section 4.1.
    ...
    *
    * Per RFC 6265, malformed `Expires`/`Max-Age` values and unknown attributes are silently
    * ignored.
    *
  ...
  /** Validates cookie name per RFC 6265 Section 4.1.1.
    *
    * cookie-name = token (no control chars, separators, etc.), using the token production from RFC
    * 2616.
    */
  ...
  /** Validates cookie value per RFC 6265 Section 4.1.1.
    ...
    *
    * An empty value is allowed. A value wrapped in DQUOTEs is validated on its inner content.
    */
