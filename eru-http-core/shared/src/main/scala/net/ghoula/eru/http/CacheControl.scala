...
enum CacheDirective {
  ...
  /** Indicates that the response is intended for a single user and must not be stored by a shared
    * cache (RFC 9111 Section 5.2.2.6).
    *
    * @param fields
    *   optional list of field names; when present, a shared cache must not store these fields but
    *   may store the rest of the response
    */
...
final case class CacheControl(directives: List[CacheDirective]) {
  ...
  /** Checks if this cache control contains a specific directive type.
    *
    * Note: This checks by directive type, ignoring values. For example,
    * has(CacheDirective.MaxAge(3600)) will return true if any MaxAge directive exists, regardless
...
object CacheControl {
  ...
  /** Parses a Cache-Control header value.
    ...
    * handles comma-separated directives. Directives are parsed in order and fail fast on the first
    * malformed directive.
    *
    * @param value
    *   the Cache-Control header value
