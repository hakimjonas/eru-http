...
object Method {
  ...
  /** QUERY method: Initiates a safe, idempotent query that carries the query as request content.
    * RFC 10008.
    */
  ...
  extension (method: Method) {
    ...
    /** A safe method does not change server state. RFC 9110 Section 9.2.1
      *
      * @return
      *   true if the method is safe (GET, HEAD, OPTIONS, TRACE, QUERY)
      */
    def isSafe: Boolean = method match {
      case GET | HEAD | OPTIONS | TRACE | QUERY => true
      case _ => false
    }
    ...
    /** An idempotent method can be called multiple times with the same effect. RFC 9110 Section
      * 9.2.2
      *
      * QUERY is idempotent per RFC 10008 Section 2: a QUERY request can be automatically repeated
      * or restarted without concern for partial state changes.
      *
      * @return
      *   true if the method is idempotent (GET, HEAD, PUT, DELETE, OPTIONS, TRACE, QUERY)
      */
    def isIdempotent: Boolean = method match {
      case GET | HEAD | PUT | DELETE | OPTIONS | TRACE | QUERY => true
      case _ => false
    }

    /** Whether responses to this method are cacheable by default, per RFC 9111 Section 3.
      *
      * QUERY responses are cacheable per RFC 10008 Section 2.7; the cache key must incorporate the
      * request content and its metadata. POST responses are NOT cacheable by default: RFC 9111
      * makes them cacheable only with explicit freshness information, which a method-only predicate
      * cannot observe — check the response's Cache-Control for that.
      *
      * @return
      *   true if the method allows caching by default (GET, HEAD, QUERY)
      */
    def isCacheable: Boolean = method match {
      case GET | HEAD | QUERY => true
      case _ => false
    }

    /** Whether this method allows a request body. RFC 9110 Section 9
      *
      * QUERY carries request content (the query), so it allows a body.
      *
      * @return
