...
object WebSocketFrame {
  ...
  /** Ping frame for connection keepalive.
    ...
    * frame, an endpoint MUST send a Pong frame in response with the same payload.
    *
    * @param data
  ...
  /** Create a raw frame from components.
    *
    * This is primarily for internal use by the frame parser.
    */
  private[websocket] def fromRaw(
    ...
    case WebSocketOpcode.Close =>
      ...
      else if payload.length == 1 then Close(None, None) // Invalid per RFC, treat as no code
      else {
