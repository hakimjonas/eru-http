...
object ContentEncoding {
  ...
  /** Parses multiple content encodings from a comma-separated string.
    *
    * Per RFC 9110, multiple encodings can be listed in order of application. For example, "gzip,
    * deflate" means the content was first deflated, then gzipped.
    *
    * @param value
  ...
  def parseAcceptEncoding(value: String): Eru[ParseError, List[(ContentEncoding, Double)]] = {
    ...
    if parts.isEmpty then {
      ...
      // Parse each encoding with optional qvalue
  ...
  /** Parses encoding or wildcard (*).
    */
  private def parseEncodingOrWildcard(value: String): Eru[ParseError, ContentEncoding] = {
    if value == "*" then {
      Eru.succeed(ContentEncoding.Identity) // Wildcard maps to identity
    } else {
      parse(value)
    }
  }
  ...
  private def extractQValue(params: List[String]): Eru[ParseError, Double] = {
    params.find { p =>
      ...
      .getOrElse(Eru.succeed(1.0)) // Default qvalue
  }
  ...
  extension (encoding: ContentEncoding) {
    ...
    /** Returns true if this encoding is supported for compression/decompression.
      *
      * JVM Platform Support:
      *   - Gzip: Always supported (java.util.zip)
      *   - Deflate: Always supported (java.util.zip)
      *   - Brotli: Supported if brotli4j native library loads successfully
      *   - Identity: Always supported (no-op)
      *
      * JS Platform: Not yet implemented (pending Eru JS support)
      *
      * @return
    ...
    def isSupported: Boolean = encoding match {
      ...
      case ContentEncoding.Brotli => true // Brotli4j library available on JVM
      case _ => false
    }
  }
