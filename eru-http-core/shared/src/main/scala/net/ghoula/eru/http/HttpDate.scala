...
object HttpDate {
  ...
  def parse(value: String): Eru[ParseError, Instant] = {
    ...
    // Try IMF-fixdate format first (preferred)
  ...
  // IMF-fixdate format (preferred): Sun, 06 Nov 1994 08:49:37 GMT
  ...
  // RFC 850 format (obsolete): Sunday, 06-Nov-94 08:49:37 GMT
  // Note: 2-digit years are ambiguous; we use a pivot year approach (00-49 -> 2000-2049, 50-99 -> 1950-1999)
  private val rfc850Formatter: DateTimeFormatter = {
    new DateTimeFormatterBuilder()
      .parseCaseInsensitive()
      .appendPattern("EEEE, dd-MMM-")
      .appendValueReduced(ChronoField.YEAR, 2, 2, 1950) // Pivot year: 1950
  ...
  private val asctimeFormatterAlt: DateTimeFormatter = {
    new DateTimeFormatterBuilder()
      .parseCaseInsensitive()
      .appendPattern("EEE MMM  ") // Two spaces for single-digit days
  ...
  /** Checks if a resource has been modified since a given date.
    *
    * Used for implementing If-Modified-Since logic.
    *
    * @param lastModified
  ...
  def isModifiedSince(lastModified: Instant, ifModifiedSince: Instant): Boolean = {
    // Per RFC 9110, we truncate to seconds for comparison (HTTP dates don't have sub-second precision)
