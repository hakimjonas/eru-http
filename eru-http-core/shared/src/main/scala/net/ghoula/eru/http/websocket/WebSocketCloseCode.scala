...
object WebSocketCloseCode {
  ...
  /** Create a WebSocketCloseCode from an integer with validation.
    *
    * Per RFC 6455 Section 7.4 and the IANA registry, valid close codes are:
    *   - 1000-1011: Protocol-defined codes (RFC 6455 Section 7.4.1)
    *   - 1012-1014: IANA-registered codes (Service Restart, Try Again Later, Bad Gateway)
  ...
  private def isValidCode(value: Int): Boolean = {
    ...
    else if value >= 1016 && value <= 2999 then false
  ...
  extension (code: WebSocketCloseCode) {
    ...
    /** Whether this is a reserved code that should not be sent. 1004 is reserved but unused.
      */
