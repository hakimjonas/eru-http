...
object Uri {
  ...
  extension (uri: Uri) {
    ...
    /** Resolves a relative URI reference against this URI per RFC 3986 Section 5.2.
      *
      * Absolute references are returned as-is (modulo dot-segment removal). Relative references
      * inherit the base scheme, authority, and path as prescribed. Per Section 5.2, an empty
      * reference resolves to the base without its fragment.
      *
      * @param relative
      *   the URI reference to resolve
      * @return
      *   the resolved URI or an InvalidUri error when the reference does not parse
      */
    def resolve(relative: String): Eru[InvalidUri, Uri] =
      if relative.isEmpty then {
        Eru.succeed(Components(uri.scheme, uri.authority, uri.path, uri.query, None))
      } else {
        Uri.parse(relative).map { r =>
          r.scheme match {
            case Some(_) =>
              Components(r.scheme, r.authority, removeDotSegments(r.path), r.query, r.fragment)
            case None =>
              r.authority match {
                case Some(_) =>
                  Components(uri.scheme, r.authority, removeDotSegments(r.path), r.query, r.fragment)
                case None =>
                  if r.path.isEmpty then {
                    Components(uri.scheme, uri.authority, uri.path, r.query.orElse(uri.query), r.fragment)
                  } else {
                    val mergedPath =
                      if r.path.startsWith("/") then r.path
                      else mergePaths(uri, r.path)
                    Components(uri.scheme, uri.authority, removeDotSegments(mergedPath), r.query, r.fragment)
                  }
              }
          }
        }
    ...
    /** Changes the scheme.
      *
      * Validates that the scheme is non-empty and contains only scheme characters (letters, digits,
      * '+', '-', '.'), then lowercases it. This is weaker than the RFC 3986 scheme grammar, which
      * requires the first character to be a letter (Section 3.1).
      */
  ...
  /** URL encodes a string per RFC 3986.
    ...
    * percent-encoded byte-by-byte from their UTF-8 encoding; the underlying byte array is read
    * without copying (hoisted once rather than per nibble).
    */
  private def encode(s: String): String = {
    ...
    s.foreach { c =>
      if isUnreserved(c) then {
        ...
        val arr = Bytes.fromString(c.toString, Charset.UTF8).unsafeArray
        var i = 0
        while i < arr.length do {
          result.append('%')
          result.append(toHex((arr(i) >> 4) & 0x0f))
          result.append(toHex(arr(i) & 0x0f))
          i += 1
        }
      }
    }
  ...
  /** Checks if a character is unreserved per RFC 3986. unreserved = ALPHA / DIGIT / "-" / "." / "_"
    * / "~" — ALPHA and DIGIT are ASCII-only, so non-ASCII letters and digits are percent-encoded
    * from their UTF-8 encoding.
    */
  private def isUnreserved(c: Char): Boolean =
    (c.isLetterOrDigit && c < 0x80) || c == '-' || c == '.' || c == '_' || c == '~'
  ...
  /** RFC 3986 Section 5.3: merge the base path with a relative path. */
  private def mergePaths(base: Uri, relativePath: String): String = {
    if base.authority.isDefined && base.path.isEmpty then "/" + relativePath
    else {
      val lastSlash = base.path.lastIndexOf('/')
      if lastSlash < 0 then relativePath
      else base.path.substring(0, lastSlash + 1) + relativePath
    }
  }

  /** RFC 3986 Section 5.2.4: remove dot segments from a path. */
  private def removeDotSegments(path: String): String = {
    val input = new StringBuilder(path)
    val output = new StringBuilder

    while input.nonEmpty do {
      val s = input.toString
      if s.startsWith("../") then input.delete(0, 3)
      else if s.startsWith("./") then input.delete(0, 2)
      else if s.startsWith("/./") then input.replace(0, 3, "/")
      else if s == "/." then input.replace(0, 2, "/")
        ...
        input.replace(0, 3, "/")
        removeLastSegment(output)
      } else if s == "." || s == ".." then input.clear()
      else {
        val slash = s.indexOf('/', 1)
        if slash < 0 then {
          output.append(s)
          input.clear()
        } else {
          output.append(s.substring(0, slash))
          input.delete(0, slash)
        }
      }
    }
  ...
  private def removeLastSegment(output: StringBuilder): Unit = {
    val slash = output.lastIndexOf("/")
    if slash >= 0 then output.setLength(slash)
    else output.clear()
  }
