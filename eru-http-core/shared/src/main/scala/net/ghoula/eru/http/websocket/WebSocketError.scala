...
/** WebSocket-specific errors as defined by protocol requirements in RFC 6455.
  *
  * These errors cover all failure modes in WebSocket communication:
...
enum WebSocketError {
  ...
  def suggestedCloseCode: Option[WebSocketCloseCode] = this match {
    ...
    case HandshakeFailed(_, _) => None // Handshake failures happen before WebSocket is established
    case ConnectionClosed(_, _, _) => None // Already closed
  }
}
