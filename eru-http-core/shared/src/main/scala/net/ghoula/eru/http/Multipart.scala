...
/** Multipart form data support per RFC 7578.
  ...
  * )
  * val multipart = Multipart.formData(parts).unsafeRunSync()
  * val body = multipart.toBody.unsafeRunSync()
  * }}}
  */
...
/** Multipart form data per RFC 7578.
  *
  * A collection of parts separated by a boundary string. Each part can be a form field or a file
  * upload.
  *
  * @param parts
...
final case class Multipart(
  ...
  /** Converts this multipart message to a Body.
    *
    * Encodes all parts using the multipart/form-data format with CRLF line endings. Streaming parts
    * are not supported and produce a typed EncodeError; other body shapes are pure conversions.
    * Part bodies are encoded as ISO-8859-1 so arbitrary bytes round-trip through String without
    * corruption.
    *
    * @return
  ...
  def toBody: Eru[HttpError, Body] = {
    if parts.isEmpty then {
      ...
      val partBytesEru: Eru[HttpError, List[Bytes]] = Eru.foreach(parts) { part =>
        part.body match {
          ...
          case _: Body.Stream =>
            Eru.fail(
              HttpError.BodyEncodeError(EncodeError("Streaming parts not yet supported in multipart encoding"))
            )
        }
      }

      partBytesEru.map { partBytesList =>
        val builder = new StringBuilder

        parts.zip(partBytesList).foreach { case (part, bodyBytes) =>
          builder.append("--").append(boundary).append("\r\n")
          builder.append("Content-Disposition: ").append(part.contentDispositionValue).append("\r\n")
          part.contentType.foreach { ct =>
            builder.append("Content-Type: ").append(ct.value).append("\r\n")
          }
          builder.append("\r\n")
          builder.append(bodyBytes.asString(Charset.ISO_8859_1)).append("\r\n")
        }
...
object Multipart {
  ...
  /** Creates a multipart/form-data message from parts.
    ...
    * @return
    *   an Eru effect producing the Multipart, or a ProtocolError when parts is empty
    */
  ...
  /** Parses a multipart/form-data body.
    ...
    * @return
    *   an Eru effect producing the parsed Multipart, or an error when the boundary is empty or the
    *   body cannot be read
    */
  ...
  /** Parses multipart bytes with a given boundary.
    *
    * Content is decoded as ISO-8859-1 to preserve arbitrary binary data. Raw parts are matched
    * without trimming so the original structure is kept intact.
    *
    * @param bytes
    *   the bytes to parse
    * @param boundary
  ...
  private def parseBytes(bytes: Bytes, boundary: String): Eru[HttpError, Multipart] = {
    Eru.effect {
      ...
      if partList.isEmpty then
        Eru.fail(HttpError.BodyDecodeError(DecodeError("No valid parts found in multipart body")))
      else Eru.succeed(Multipart(partList, boundary))
    }
  }

  /** Generates a random boundary string per RFC 7578.
    *
    * The boundary must not appear in any part content. The generated boundary is a prefixed 128-bit
    * SecureRandom hex string, which makes collisions with part content extremely unlikely.
    *
    * @return
