...
/** Request interceptor transforms requests before sending.
  *
  * Interceptors work with Eru effects: they can transform requests, run asynchronous operations
  * (for example, fetch a token from a cache), and participate in the typed error channel.
  *
  * @example
