...
private[client] final class InMemoryCookieJar extends CookieJar {
  ...
  override def add(uri: Uri, cookie: Cookie): Eru[HttpError, Unit] = {
    Eru.effectTotal {
      ...
      // Create cookie with effective domain and path
  ...
  override def getCookies(uri: Uri): Eru[HttpError, List[Cookie]] = {
    Eru.effectTotal {
      ...
      // Filter cookies that match domain, path, and are not expired
      ...
      domain match {
        case Some(d) =>
          path match {
            ...
            case None =>
              // Remove all cookies with this name and domain
        ...
        case None =>
          // Remove all cookies with this name
  ...
  private def defaultPath(uriPath: String): String = {
    if uriPath.isEmpty || !uriPath.startsWith("/") then {
      ...
      if lastSlash == 0 then {
        "/" // Only one slash at the beginning
      } else {
