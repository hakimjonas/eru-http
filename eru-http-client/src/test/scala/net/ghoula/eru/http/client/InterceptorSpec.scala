...
/** Tests for request/response interceptors.
  *
  * The Basic auth test asserts the literal base64 encoding of "user:pass" (dXNlcjpwYXNz).
  */
class InterceptorSpec extends FunSuite {
  ...
  test("ResponseInterceptor.andThenAll - composes multiple response interceptors") {
    val server = TestHttpServer.create(handler = (_, _) => TestHttpServer.ResponseConfig(body = "hello"))
    try {
      HttpClient
        .scoped(HttpClientConfig.default) { client =>
          def addResponseHeader(name: String, value: String): ResponseInterceptor = resp =>
            resp.addHeader(name, value).mapError {
              case e: HeaderName.InvalidHeaderName =>
                HttpError.InvalidResponse(InvalidResponse(s"Invalid header name: ${e.message}", "RFC 9110"))
              case e: HeaderValue.InvalidHeaderValue =>
                HttpError.InvalidResponse(InvalidResponse(s"Invalid header value: ${e.message}", "RFC 9110"))
            }

          val interceptor = addResponseHeader("X-First", "1")
            .andThenAll(
              addResponseHeader("X-Second", "2"),
              addResponseHeader("X-Third", "3")
            )
          ...
          for {
            ...
            assert(response.headers.contains("X-First"))
            assert(response.headers.contains("X-Second"))
            assert(response.headers.contains("X-Third"))
            assertEquals(response.body.asString(Charset.UTF8), "hello")
          }
        }
        .assertSuccess
    } finally {
